...
const Transactions = ({navigation}: {navigation: any}) => {
  ...
  const getCurrentMilestone = async () => {
    ...
    const currMile = await fetch(
      'https://211b-103-142-31-94.in.ngrok.io/api/v1/milestone/current-milestone/d89e855f-11a9-454e-ac83-51b28cb820be',
      {method: 'GET'},
    );
  ...
  const getTransactions = async (milestoneId: string) => {
    console.log('fetching.....');
    const currMile = await fetch(
      `https://211b-103-142-31-94.in.ngrok.io/api/v1/transaction/milestone/${milestoneId}`,
      {method: 'GET'},
    );
    return currMile.json();
  };
